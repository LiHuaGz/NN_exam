...
实验使用完整 MNIST 训练集,  batch size 为 \(512\),  测试 batch size 为 \(1024\),  训练 \(10\) 个 epoch.固定随机种子为 \(42\),  
...
\(95.24\%\) 的测试准确率,  第 \(10\) 轮达到 \(97.95\%\);自然梯度从 \(93.03\%\) 稳定提升到 \(97.18\%\),  
但最终精度仍略低于 SGD.造成这种现象的主要原因是自然梯度实现中使用的是阻尼 Fisher 近似,  并且每个 batch 只用 \(8\) 次共轭梯度迭代,  
